...
| --- | --- | --- |
| `WIKI_TITLE` | required (no runtime fallback) | Wiki name in header and metadata |
| `WIKI_DESCRIPTION` | required (no runtime fallback) | Used in meta/RSS description |
| `BASE_URL` | `''` -> auto-detect from request | Must be `http(s)` URL when set |
| `THEME` | `''` (no theme) | Must match available theme directory |
| `LANGUAGE` | configured value; fallback resolves from available i18n (commonly `ko`/`en`) | Must exist in language catalog |
| `TIMEZONE` | `''` allowed -> server timezone; invalid falls back to `UTC` | Must be valid PHP timezone when set |
| `HOME_PAGE` | required (no runtime fallback) | Must pass title sanitization and filename limit |
...
| Auth/settings (`auth.php`, auth/settings handlers/templates) | `php -l wikiwikiwiki/src/auth.php` ; `php -l wikiwikiwiki/src/user.php` ; `php -l wikiwikiwiki/src/handlers/auth.php` ; `php -l wikiwikiwiki/src/handlers/settings.php` ; verify `public` and `fully_public` modes block `create_user` (UI hidden + server guard) |
