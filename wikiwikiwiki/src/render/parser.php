...
{
    $resolved = $innerHtml;
    if ($inlinePlaceholders !== [] && str_contains($resolved, '%%HTML_')) {
...
function preserve_literal_placeholder_tokens(string $content): array
{
    ...
    $content = preg_replace_callback('/%%(?:CODE|HTML)_\d+%%/', static function (array $m) use (&$placeholders, &$counter, $namespace): string {
