<section class="section">
  ...
  <div class="section-main">
    <form class="form is-fill" id="form" action="<?= url($page) ?>" method="post">
      ...
      <input type="hidden" name="original_modified_at" value="<?= html($modifiedAt ?? '') ?>">
      ...
      <?php if (!$isNew && $page !== HOME_PAGE): ?>
        <div class="field">
          ...
          <input class="input" id="new-title" type="text" name="new_title" value="<?= html($page) ?>" pattern="[^\x5C\x3C\x3E\x3A\x22\x23\x25\x7C\x3F\x2A\x5B\x5D]+" maxlength="<?= PAGE_TITLE_MAX_LENGTH ?>" placeholder="<?= t('field.placeholder.page_title') ?>">
          <p class="field-help"><?= t('field.help.page_title') ?></p>
        </div>
      <?php endif ?>
      <?php if (isset($conflict) && $conflict): ?>
        <section class="field is-fill" aria-live="polite">
          <div class="content">
            <p role="alert"><?= t('message.edit.conflict_notice') ?></p>
          </div>
      ...
      <div class="field is-fill">
        <?php if (isset($conflict) && $conflict): ?>
          <h3 class="field-title"><?= t('message.edit.your_content') ?></h3>
        <?php endif ?>
