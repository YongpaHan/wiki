<section class="section">
  ...
  <div class="section-main">
    <form class="form is-fill" id="form" method="post">
      ...
      <input type="hidden" name="overwrite_existing" value="<?= (($overwriteExisting ?? false) === true) ? '1' : '0' ?>">
      <div class="field">
      ...
      <?php if (((string) ($existingTitle ?? '')) !== ''): ?>
        <div class="field is-fill">
          <div class="content">
            <p role="alert"><?= t('message.page.overwrite_warning') ?></p>
          </div>
          <label class="hidden" for="existing-content"><?= t('message.edit.existing_content') ?></label>
          <textarea class="textarea" id="existing-content" readonly><?= html($existingContent ?? '') ?></textarea>
        </div>
      <?php endif ?>
      <div class="field is-sticky">
        <div class="buttons">
          ...
          <div>
            <button class="button is-primary" id="button-save" type="submit"><?= t((($overwriteExisting ?? false) === true) ? 'button.overwrite' : 'button.save') ?></button>
          </div>
        </div>
      </div>
    </form>
